...
gs_mode = os.getenv("gs_mode", "2d")
current_dir = os.path.dirname(os.path.abspath(__file__))
if gs_mode == "2d":
    sys.path.append(os.path.join(current_dir, "2d_gaussian_splatting"))
elif gs_mode == "3d":
    sys.path.append(os.path.join(current_dir, "3d_gaussian_splatting"))
else:
    raise ValueError("GAUSSIAN_MODE must be '2d' or '3d'.")
...
from internal.viewer import ViewerRenderer, ViewerRenderer3D, ClientThread
...
class Viewer:
    def __init__(
        ...
        self.host = host
        self.port = port
        self.gs_mode = os.getenv("gs_mode", "2d")
    ...
    def _init_models(self, iterations):
        ...
        elif self.gs_mode =="3d":
            self.viewer_renderer = ViewerRenderer3D(self.gaussian_model, self.background_color, not self.is_training)
    ...
    def _get_training_gaussians(self, new_gaussians):
        ...
        if self.gs_mode == "2d":
            ...
            self.viewer_renderer = ViewerRenderer3D(self.gaussian_model, self.background_color, self.is_training)
