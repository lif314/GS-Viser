...
class ClientThread(threading.Thread):
    ...
    def send_camera_path(self, camera_paths, fps=30):
        ...
        video_path = Path("./renders") / "render_videos" /f"{self.viewer.render_panel.render_name}.mp4"
        video_path.parent.mkdir(parents=True, exist_ok=True)
        fourcc = cv2.VideoWriter_fourcc(*'mp4v') # or use 'avc1' for H.264
        video_writer = cv2.VideoWriter(str(video_path), fourcc, 30, (image_width, image_height))
        ...
        print(f"Render Video saved to {video_path}")
